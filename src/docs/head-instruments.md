...
для получения списка облигаций используется метод [Bonds](/investAPI/instruments#bonds), а для получения 
информации по конкретной облигации метод [BondBy](/investAPI/instruments#bondby). 
...
Такая информация предоставляется методом [TradingSchedules](/investAPI/instruments#tradingschedules). 
...
Метод [TradingSchedules](/investAPI/instruments#tradingschedules) позволяет получать расписание торгов
...
**Важно!!!** Метод [TradingSchedules](/investAPI/instruments#tradingschedules) возвращает торговое расписание на секции биржи
...
* [BondBy](/investAPI/instruments/#bondby) - Метод получения облигации по её идентификатору.
* [Bonds](/investAPI/instruments/#bonds) - Метод получения списка облигаций.
* [ShareBy](/investAPI/instruments/#shareby) - Метод получения акции по её идентификатору.
* [Shares](/investAPI/instruments/#shares) - Метод получения списка акций.
* [EtfBy](/investAPI/instruments/#etfby) - Метод получения инвестиционного фонда по его идентификатору.
* [Etfs](/investAPI/instruments/#etfs) - Метод получения списка инвестиционных фондов.
* [FutureBy](/investAPI/instruments/#futureby) - Метод получения фьючерса по его идентификатору.
* [Futures](/investAPI/instruments/#futures) - Метод получения списка фьючерсов.
* [OptionBy](/investAPI/instruments/#optionby) - Метод получения опциона по его идентификатору.
* [OptionsBy](/investAPI/instruments/#optionsby) и [Options](/investAPI/instruments/#options) - Методы получения списка опционов.
* [CurrencyBy](/investAPI/instruments/#currencyby) - Метод получения валюты по её идентификатору.
* [Currencies](/investAPI/instruments/#currencies) - Метод получения списка валют.
...
* [CurrencyBy](/investAPI/instruments/#currencyby) - Метод получения валюты по её идентификатору.
* [Currencies](/investAPI/instruments/#currencies) - Метод получения списка валют.
* [Indicatives](/investAPI/instruments/#indicatives) - Метод получения списка индекса и товаров.
...
Есть возможность получить информацию об активах, которы не торгуются на бирже, например, по индексу IMOEX или нефти.
Для этого создан метод [Indicatives](/investAPI/instruments/#indicatives).
Получив идентификатор такого актива, далее возможно построить свечи, используя метод [getCandles](/investAPI/marketdata/#getcandles), как для других инструментов.
Для построения свечей по индикативам лучше использовать идентификатор в виде UID.
...
Метод [getAssetFundamentals](/investAPI/instruments/#getassetfundamentals) возвращает показатели компании по активу, как правило, по акции компании.
Обычно обновление данных происходит на следующий рабочий день после публикации, в редких случаях это может занять более недели. Информация доступна не по всем активам.

Ряд фундаментальных показателей доступных в методе может не публиковаться компаниями или быть недоступными.
...
`price_to_sales_ttm` - не рассчитывается в отсутствии выручки;

`ebitda_ttm` - не считается для некоторых секторов экономики.

Следует отметить, что в реализации API метод возвращает все параметры контракта, значение `0` в ответе следует приравнивать к отсутствию данных. 
