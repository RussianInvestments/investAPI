...
Параметры `initial_margin_on_buy` и `initial_margin_on_sell` возвращают Гарантийное обеспечение, рассчитанное Биржей, а коэффициенты `dshort` и `dlong` определяют сумму заблокированных брокером средств при торговле фьючерсами.
...
>**Важно** <br>
>Точную сумму денежных средств блокируемых брокером можно посчитать используя параметры `dshort` и `dlong`, которые определяют коэффициент блокируемых средств от суммы сделки.
